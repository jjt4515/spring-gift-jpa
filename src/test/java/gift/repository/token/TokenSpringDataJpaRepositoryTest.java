...
public class TokenSpringDataJpaRepositoryTest {
    ...
    public void testSaveToken() {
        ...
        TokenAuth token = new TokenAuth("test-token", member.getId());
        tokenRepository.save(token);

        Optional<TokenAuth> foundToken = tokenRepository.findByToken("test-token");
        assertThat(foundToken).isPresent();
        assertThat(foundToken.get().getMemberId()).isEqualTo(member.getId());
    }
    ...
    public void testFindToken() {
        ...
        TokenAuth token = new TokenAuth("test-token-2", member.getId());
        tokenRepository.save(token);
    ...
    public void testDeleteToken() {
        ...
        TokenAuth token = new TokenAuth("test-token-3", member.getId());
        tokenRepository.save(token);
